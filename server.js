...
// ============================================================
// GRC Maturity Assessment Module (v37)
// Access: Admin — full management; CRO / Consultant CRO — run + view
// ============================================================

// ── Scoring helpers ────────────────────────────────────────────────────────

function roundToLevel(score) {
    // Standard rounding: 0.5 and above rounds up, below 0.5 rounds down.
    return Math.min(5, Math.max(1, Math.round(score)));
}

async function computeMaturityResults(client, assessmentId, companyId) {
    // Load all active domains for this company (with weights)
    const domainsRes = await client.query(
        `SELECT id, name, weight FROM maturity_domains
    ...
    const responsesRes = await client.query(
        `SELECT r.domain_id, r.selected_level
    ...
    for (const row of responsesRes.rows) {
        if (!byDomain[row.domain_id]) byDomain[row.domain_id] = [];
        byDomain[row.domain_id].push(row.selected_level);
    }

    const totalWeight = domains.reduce((s, d) => s + parseFloat(d.weight), 0) || 100;

    const domainScores = domains.map((d) => {
        const levels = byDomain[d.id] || [];
        ...
        return {
            ...
            level: levels.length > 0 ? roundToLevel(score) : 0,
            question_count: levels.length,
        };
    });

    // Weighted overall score
    const weightedSum = domainScores.reduce((s, d) => s + d.score * d.weight, 0);
    const overallScore = Math.round((weightedSum / totalWeight) * 100) / 100;
    const overallLevel = roundToLevel(overallScore);

    return { overallScore, overallLevel, domainScores };
}

// ── Domains ────────────────────────────────────────────────────────────────

// GET /api/maturity/domains
// Returns all active domains for the current company, with question counts.
app.get(
    '/api/maturity/domains',
    requireRole('Admin', 'CRO'),
    asyncHandler(async (req, res) => {
        const result = await pool.query(
            `SELECT d.id, d.name, d.description, d.weight, d.display_order, d.is_active,
                    COUNT(q.id) FILTER (WHERE q.is_active = true) AS active_question_count
             FROM maturity_domains d
             LEFT JOIN maturity_questions q ON q.domain_id = d.id
             WHERE d.company_id = $1
             GROUP BY d.id
...
// POST /api/maturity/domains
app.post(
    '/api/maturity/domains',
    requireRole('Admin'),
    asyncHandler(async (req, res) => {
        const { name, description, weight, display_order } = req.body;
        if (!name || !name.trim()) return res.status(400).json({ error: 'name is required' });
        const w = parseFloat(weight) || 20;
        if (w <= 0 || w > 100) return res.status(400).json({ error: 'weight must be between 0 and 100' });

        // Auto-assign display_order if not provided
        const orderRes = await pool.query(
            `SELECT COALESCE(MAX(display_order), 0) + 1 AS next_order
             FROM maturity_domains WHERE company_id = $1`,
            [req.company.id]
        );
        const order = display_order != null ? parseInt(display_order) : orderRes.rows[0].next_order;

        const result = await pool.query(
            `INSERT INTO maturity_domains (company_id, name, description, weight, display_order)
             VALUES ($1, $2, $3, $4, $5) RETURNING *`,
            [req.company.id, name.trim(), description || null, w, order]
        );
        res.status(201).json(result.rows[0]);
    })
);

// PUT /api/maturity/domains/:id
app.put(
    '/api/maturity/domains/:id',
    requireRole('Admin'),
    asyncHandler(async (req, res) => {
        const { name, description, weight, display_order, is_active } = req.body;
        const result = await pool.query(
...
// DELETE /api/maturity/domains/:id  (soft delete)
app.delete(
    '/api/maturity/domains/:id',
    requireRole('Admin'),
    asyncHandler(async (req, res) => {
        const result = await pool.query(
            `UPDATE maturity_domains SET is_active = false, updated_at = now()
...
// GET /api/maturity/domains/:domainId/questions
app.get(
    '/api/maturity/domains/:domainId/questions',
    requireRole('Admin', 'CRO'),
    asyncHandler(async (req, res) => {
        // Verify domain belongs to this company
        const domainCheck = await pool.query(
            `SELECT id FROM maturity_domains WHERE id = $1 AND company_id = $2`,
            [req.params.domainId, req.company.id]
        );
        if (!domainCheck.rows.length) return res.status(404).json({ error: 'Domain not found' });

        const result = await pool.query(
            `SELECT id, domain_id, question_text, option_1, option_2, option_3, option_4, option_5,
                    display_order, is_active, created_at
             FROM maturity_questions
             WHERE domain_id = $1 AND company_id = $2
             ORDER BY display_order, id`,
            [req.params.domainId, req.company.id]
        );
        res.json(result.rows);
...
// POST /api/maturity/questions
app.post(
    '/api/maturity/questions',
    requireRole('Admin'),
    asyncHandler(async (req, res) => {
        const { domain_id, question_text, option_1, option_2, option_3, option_4, option_5 } = req.body;
        ...
        if (!option_1 || !option_2 || !option_3 || !option_4 || !option_5) {
            return res.status(400).json({ error: 'All five option texts are required' });
        }

        // Verify domain belongs to this company
        const domainCheck = await pool.query(
            `SELECT id FROM maturity_domains WHERE id = $1 AND company_id = $2 AND is_active = true`,
            [domain_id, req.company.id]
        );
        if (!domainCheck.rows.length) return res.status(404).json({ error: 'Domain not found' });

        const orderRes = await pool.query(
            `SELECT COALESCE(MAX(display_order), 0) + 1 AS next_order
             FROM maturity_questions WHERE domain_id = $1`,
            [domain_id]
        );

        const result = await pool.query(
            `INSERT INTO maturity_questions
                (domain_id, company_id, question_text, option_1, option_2, option_3, option_4, option_5, display_order)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`,
            [
                domain_id, req.company.id,
                question_text.trim(),
                option_1.trim(), option_2.trim(), option_3.trim(), option_4.trim(), option_5.trim(),
...
// PUT /api/maturity/questions/:id
app.put(
    '/api/maturity/questions/:id',
    requireRole('Admin'),
    asyncHandler(async (req, res) => {
        const { question_text, option_1, option_2, option_3, option_4, option_5, display_order, is_active } = req.body;
        const result = await pool.query(
            `UPDATE maturity_questions
             SET question_text  = COALESCE($1, question_text),
                 option_1       = COALESCE($2, option_1),
                 option_2       = COALESCE($3, option_2),
                 option_3       = COALESCE($4, option_3),
                 option_4       = COALESCE($5, option_4),
                 option_5       = COALESCE($6, option_5),
            ...
            [
                ...
                option_1?.trim() || null, option_2?.trim() || null,
                option_3?.trim() || null, option_4?.trim() || null, option_5?.trim() || null,
                display_order != null ? parseInt(display_order) : null,
                is_active != null ? is_active : null,
                req.params.id,
                req.company.id,
...
// DELETE /api/maturity/questions/:id  (soft delete)
app.delete(
    '/api/maturity/questions/:id',
    requireRole('Admin'),
    asyncHandler(async (req, res) => {
        const result = await pool.query(
            `UPDATE maturity_questions SET is_active = false, updated_at = now()
             WHERE id = $1 AND company_id = $2 RETURNING id`,
            [req.params.id, req.company.id]
        );
        if (!result.rows.length) return res.status(404).json({ error: 'Question not found' });
...
// Lists all assessments for this company (most recent first), with result summary.
app.get(
    ...
    asyncHandler(async (req, res) => {
        const result = await pool.query(
            `SELECT a.id, a.status, a.started_at, a.completed_at,
                    ...
                    r.overall_score, r.overall_level, r.domain_scores
             FROM maturity_assessments a
             LEFT JOIN users u ON u.id = a.started_by
             LEFT JOIN maturity_results r ON r.assessment_id = a.id
...
// POST /api/maturity/assessments  — start a new assessment
app.post(
    '/api/maturity/assessments',
    requireRole('Admin', 'CRO'),
    asyncHandler(async (req, res) => {
        // Check there are active questions to answer
        const qCheck = await pool.query(
            `SELECT COUNT(*) AS cnt FROM maturity_questions q
             JOIN maturity_domains d ON d.id = q.domain_id
             WHERE q.company_id = $1 AND q.is_active = true AND d.is_active = true`,
            [req.company.id]
        );
        if (parseInt(qCheck.rows[0].cnt) === 0) {
            return res.status(400).json({ error: 'No active questions found. Set up domains and questions first.' });
        }

        const result = await pool.query(
            `INSERT INTO maturity_assessments (company_id, started_by)
...
// GET /api/maturity/assessments/:id
// Returns the assessment with all questions (ordered by domain then display_order)
// and existing responses so the UI can restore progress on resume.
app.get(
    '/api/maturity/assessments/:id',
    requireRole('Admin', 'CRO'),
    asyncHandler(async (req, res) => {
        ...
        if (!aRes.rows.length) return res.status(404).json({ error: 'Assessment not found' });
        const assessment = aRes.rows[0];

        // Questions active at the time — use current active set
        const qRes = await pool.query(
            `SELECT q.id, q.domain_id, q.question_text,
                    q.option_1, q.option_2, q.option_3, q.option_4, q.option_5,
                    q.display_order,
                    d.name AS domain_name, d.display_order AS domain_order, d.weight
             FROM maturity_questions q
             JOIN maturity_domains d ON d.id = q.domain_id
             WHERE q.company_id = $1 AND q.is_active = true AND d.is_active = true
             ORDER BY d.display_order, d.id, q.display_order, q.id`,
            [req.company.id]
        );

        // Existing responses
        const rRes = await pool.query(
            `SELECT question_id, selected_level, commentary, answered_at
             FROM maturity_responses WHERE assessment_id = $1`,
            [req.params.id]
        );
        const responses = {};
        for (const r of rRes.rows) responses[r.question_id] = r;

        // Result (if completed)
        const resultRes = await pool.query(
            `SELECT overall_score, overall_level, domain_scores
             FROM maturity_results WHERE assessment_id = $1`,
            [req.params.id]
        );
...
// POST /api/maturity/assessments/:id/responses  — save / update a single response
app.post(
    '/api/maturity/assessments/:id/responses',
    requireRole('Admin', 'CRO'),
    asyncHandler(async (req, res) => {
        const { question_id, selected_level, commentary } = req.body;

        if (!question_id) return res.status(400).json({ error: 'question_id is required' });
        if (!selected_level || selected_level < 1 || selected_level > 5) {
            return res.status(400).json({ error: 'selected_level must be 1–5' });
        }

        // Verify assessment belongs to this company and is in progress
        const aRes = await pool.query(
            `SELECT id, status FROM maturity_assessments WHERE id = $1 AND company_id = $2`,
            [req.params.id, req.company.id]
        );
        if (!aRes.rows.length) return res.status(404).json({ error: 'Assessment not found' });
        if (aRes.rows[0].status === 'completed') {
            return res.status(400).json({ error: 'Assessment is already completed' });
        }

        // Verify question belongs to this company
        const qRes = await pool.query(
            `SELECT id, domain_id FROM maturity_questions WHERE id = $1 AND company_id = $2`,
            [question_id, req.company.id]
        );
        if (!qRes.rows.length) return res.status(404).json({ error: 'Question not found' });

        // Upsert response
        const result = await pool.query(
            `INSERT INTO maturity_responses (assessment_id, question_id, domain_id, selected_level, commentary, answered_at)
             VALUES ($1, $2, $3, $4, $5, now())
             ON CONFLICT (assessment_id, question_id)
             DO UPDATE SET selected_level = EXCLUDED.selected_level,
                           commentary     = EXCLUDED.commentary,
                           answered_at    = now()
             RETURNING *`,
            [req.params.id, question_id, qRes.rows[0].domain_id, selected_level, commentary || null]
        );
        res.json(result.rows[0]);
    })
);

// POST /api/maturity/assessments/:id/complete  — compute scores and mark done
app.post(
    '/api/maturity/assessments/:id/complete',
    requireRole('Admin', 'CRO'),
    asyncHandler(async (req, res) => {
        const client = await pool.connect();
        try {
            ...
            // Verify all active questions are answered
            const unansweredRes = await client.query(
                `SELECT COUNT(*) AS cnt
                 FROM maturity_questions q
                 JOIN maturity_domains d ON d.id = q.domain_id
                 LEFT JOIN maturity_responses r ON r.question_id = q.id AND r.assessment_id = $1
                 WHERE q.company_id = $2 AND q.is_active = true AND d.is_active = true
                   AND r.id IS NULL`,
                [req.params.id, req.company.id]
            );
            if (parseInt(unansweredRes.rows[0].cnt) > 0) {
                await client.query('ROLLBACK');
                return res.status(400).json({
                    error: `${unansweredRes.rows[0].cnt} question(s) still unanswered`,
                });
            }

            // Compute scores
            const { overallScore, overallLevel, domainScores } = await computeMaturityResults(
                client, req.params.id, req.company.id
            );

            // Store results
            await client.query(
                `INSERT INTO maturity_results (assessment_id, company_id, overall_score, overall_level, domain_scores)
                 VALUES ($1, $2, $3, $4, $5)
                 ON CONFLICT (assessment_id) DO UPDATE
                 SET overall_score = EXCLUDED.overall_score,
                     overall_level = EXCLUDED.overall_level,
                     domain_scores = EXCLUDED.domain_scores`,
                [req.params.id, req.company.id, overallScore, overallLevel, JSON.stringify(domainScores)]
            );
            ...
            await client.query('COMMIT');
            res.json({ overall_score: overallScore, overall_level: overallLevel, domain_scores: domainScores });
        } catch (e) {
            await client.query('ROLLBACK');
            throw e;
        } finally {
